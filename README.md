...
23 tools across accounts, calendars, events, tasks, and tags — 7 read-only, 16 mutating:
...
Pass `--read-only` (or set `MORGENMCP_READ_ONLY=1`) to expose only the 7 read-only tools. For the exact live list with schemas and annotations, connect any MCP client or run the [MCP Inspector](#local-debugging-with-mcp-inspector).
...
| `MORGENMCP_READ_ONLY` | `--read-only` | off | Truthy (`1`/`true`/`yes`/`on`) exposes only the 7 read tools, disabling all 16 mutating ones. |
