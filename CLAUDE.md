...
- **Bounded batch concurrency**: batch/fan-out `asyncio.gather` calls in `tools/` go through `gather_bounded` (`tools/utils.py`, semaphore, `BATCH_CONCURRENCY = 8`, always `return_exceptions=True`) — applies to `batch_delete_events`, `batch_update_events`, `batch_delete_tasks`, and the per-account fan-out in `tools/events.py::list_events`. Never add a bare `asyncio.gather` over per-item API calls in `tools/`. **Known exception**: `resources.py::_fetch_events_in_window` (backing every `morgen://events/*` and `morgen://calendar/{id}/events` resource) does its own per-account `asyncio.gather` fan-out *without* `gather_bounded` — it mirrors the tool-side pattern but was never given the same bound. Treat this as an open gap, not a second accepted pattern, when touching that function.
