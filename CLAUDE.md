...
- **Compact event kind tags** (`_classify_event` in `tools/events.py`): a compact line may carry a kind tag **inside the ID brackets** — `[abc123 task]` (`metadata.taskId`), `routine` (`metadata.canBeCompleted`), `flexible` (`metadata.isAutoScheduled`), `free` (`freeBusyStatus == "free"`), or a comma-joined pair such as `[abc123 task,free]` (**no space after the comma**). The tag lives inside the brackets deliberately: an earlier `{tag}`-before-`[id]` form could be **spoofed by a title ending in a brace-word** (`"Board meeting {free}"` on a busy event rendered byte-identically to a genuinely free one, so a client would schedule over a hard conflict). Inside the brackets, a spoof would require forging the 7-char virtual ID. The first three are mutually exclusive and evaluated in that order; `free` is orthogonal. **An ordinary busy event is deliberately untagged** — it was 79% of a 339-event live listing, and compact mode exists to save tokens, so absence means "normal commitment". Applied at all five `_format_compact_event` return sites (all-day, floating, bad-tz, normal, parse-failure) so the format is uniform, and inherited by all four event resources via `_events_payload` (`morgen://events/today`, `/this-week`, `/upcoming`, plus `morgen://calendar/{id}/events`). **Parsing:** match the trailing `[<7-char id>( <tag>)?]` at end of line. Titles may contain braces and brackets (9 of 388 real titles contain `[...]`), so anchor on the line end; never test for a tag with `assert "{" not in result`.
- **Undocumented `morgen.so:metadata` fields**: `canBeCompleted`, `isAutoScheduled` and `isFlexible` are returned by the live API on every event carrying a metadata object, but appear in **no commit** of the docs repo and not on docs.morgen.so. Their meaning comes from Morgen's product docs: `canBeCompleted` marks a [Routine](https://www.morgen.so/routines) — a recurring block you check off, "displayed like tasks but do not clutter the task sidebar" — and is **mutually exclusive with `taskId`** upstream ("An event with canBeCompleted cannot have a taskId"), which is why the classifier falls through `task` → `routine`. All three are modeled `bool | None = None` so "absent" stays distinguishable from "explicitly false"; if Morgen drops one, its tag silently stops appearing rather than erroring. `isFlexible` is modeled but nothing classifies on it — it was `false` on all 459 probed events and its meaning is unconfirmed. **Do not model `frameFilterMql`**: it embeds raw Apple Reminders list UUIDs, and leaving it unmodeled keeps Pydantic's `extra="ignore"` dropping it.
- **`morgen.so:metadata` is writable on create, despite being documented read-only.** `events.mdx:201` calls it "Read-only Morgen-specific metadata", but a live probe (2026-09-11) created an event with `"morgen.so:metadata": {"canBeCompleted": true}` and Morgen accepted it (HTTP 200) and echoed it back. Setting `canBeCompleted` also causes Morgen to **auto-populate `progress: "needs-action"`** — the completion-tracking field — confirming `canBeCompleted` is the "check-off-able" flag. `canBeCompleted` is metadata-only: sent as a **top-level** field it is rejected with `"property canBeCompleted should not exist"` (create whitelists properties rather than ignoring unknowns). Treat this as doc drift, not a guarantee: it is undocumented behavior that could change.
- **Routines expand like any recurring event.** A recurring routine (`recurrenceRules` + `metadata.canBeCompleted`, `count` is accepted) expands to one event per occurrence, and **every occurrence carries `canBeCompleted: true` independently** — so `_classify_event` tags each one `{routine}` with no master-event lookup. Each occurrence gets its own `id` (and so its own virtual ID) plus a shared `masterEventId` and its own `recurrenceId`. Note occurrences report `isRecurring: false` in full output, because `_format_full_event` derives that from `recurrence_rules`, which the expanded occurrences do not carry — the series is identified by `masterEventId`/`recurrenceId` instead. Deleting the whole series is `POST /events/delete?seriesUpdateMode=all` with the **master** id; it returns 204 with an empty body. Creating one is also possible via the API (`recurrenceRules` + `metadata.canBeCompleted`); `count` is accepted in the rule.
- **`canBeCompleted` + `taskId` is rejected upstream** with HTTP 400 `"An event with canBeCompleted cannot have a taskId"` — reproduced first-hand on 2026-09-11, not just reported by third parties. This is why `_classify_event` falls through `task` → `routine`: the two cannot co-occur, so the ordering is a tiebreak Morgen can never actually exercise.
- **Completing a routine does not clear `canBeCompleted`.** A checked-off routine keeps `canBeCompleted: true` and sets `progress: "completed"` (verified by read-back; note the `/events/update` *response* returns `morgen.so:metadata: null`, so re-read from `/events/list` rather than trusting the update echo). The `{routine}` tag therefore persists after check-off — it marks what the item is, while `progress` carries done-ness. This is why `progress` is surfaced in full output.
- **`isRecurring` is wrong for expanded occurrences** (pre-existing, not introduced by the kind-tag work): `_format_full_event` derives it from `recurrence_rules`, which occurrences do not carry, so every occurrence of every recurring event reports `isRecurring: false`. The series is identified by `masterEventId`/`recurrenceId` instead. Fixing it means deriving from those fields; left alone here to keep the tag change scoped.
