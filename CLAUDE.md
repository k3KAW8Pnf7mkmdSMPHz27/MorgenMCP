...
- **Compact event kind tags** (`_classify_event` in `tools/events.py`): a compact line may carry a `{kind}` tag immediately before the `[id]` — `{task}` (`metadata.taskId`), `{routine}` (`metadata.canBeCompleted`), `{flexible}` (`metadata.isAutoScheduled`), `{free}` (`freeBusyStatus == "free"`), or a comma-joined pair such as `{task,free}` (**no space** — the tag must stay one whitespace-delimited token). The first three are mutually exclusive and evaluated in that order; `free` is orthogonal. **An ordinary busy event is deliberately untagged** — it is ~86% of a typical listing, and compact mode exists to save tokens, so absence means "normal commitment". Applied at all five `_format_compact_event` return sites (all-day, floating, bad-tz, normal, parse-failure) so the format is uniform, and inherited by all four `morgen://events/*` resources via `_events_payload`. **Parsing:** titles may contain braces (and 9 of 388 real titles contain `[...]`), so the tag is only well defined as the final `{...}` immediately preceding the trailing `" [id]"` — parse right-to-left. Never test for it with `assert "{" not in result`; anchor on the line end instead.
